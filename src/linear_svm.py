...
def apply_linear_svm(train_data, test_data):
...
def evaluate_model(targets, predictions,proba):
    ...
    path = "../results/results_linear_svm.yaml"
    with open(path, "w") as f:
        yaml.dump(report, f)

def main():
    print("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    print(f"IDH Prediction using Linear SVM")
    train_data, test_data = create_train_test_split()
    y_pred, proba = apply_linear_svm(train_data, test_data)
    targets = test_data[1]
