...
def apply_logistic_regr(train_data, test_data):
    x_train, y_train = train_data
    x_test, y_test = test_data
    
    clf = LogisticRegression(max_iter=5000,solver = "saga", random_state=SEED)
    clf.fit(x_train, y_train)
    path = "../models/logistic_regr.pkl"
...
def evaluate_model(targets, predictions,proba):
    ...
    path = "../results/results_logistic.yaml"
    with open(path, "w") as f:
        yaml.dump(report, f)

def main():
    print("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    print(f"IDH Prediction using Logistic Regression")
    train_data, test_data = create_train_test_split()
    y_pred, proba = apply_logistic_regr(train_data, test_data)
    targets = test_data[1]
