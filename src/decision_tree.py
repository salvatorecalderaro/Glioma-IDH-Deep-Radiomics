...
def apply_decision_tree(train_data, test_data):
...
def evaluate_model(targets, predictions,proba):
    ...
    path = "../results/results_decision_tree.yaml"
    with open(path, "w") as f:
        yaml.dump(report, f)

def main():
    print("+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
    print(f"IDH Prediction using Decision Tree")
    train_data, test_data = create_train_test_split()
    y_pred, proba = apply_decision_tree(train_data, test_data)
    targets = test_data[1]
